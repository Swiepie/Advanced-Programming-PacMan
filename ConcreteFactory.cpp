...
std::unique_ptr<RedGhost> ConcreteFactory::createRedGhost(float x, float y) {
  ...
  auto view =
      std::make_unique<RedGhostView>(ghost.get(), window, height, width);
...
std::unique_ptr<BlueGhost> ConcreteFactory::createBlueGhost(float x, float y,
                                                            float delay) {
  auto ghost = std::make_unique<BlueGhost>(x, y, delay);

  auto view =
      std::make_unique<BlueGhostView>(ghost.get(), window, height, width);
  ghost->attach(view.get());
...
std::unique_ptr<PinkGhost> ConcreteFactory::createPinkGhost(float x, float y,
                                                            float delay) {
  auto ghost = std::make_unique<PinkGhost>(x, y, delay);

  auto view =
      std::make_unique<PinkGhostView>(ghost.get(), window, height, width);
  ghost->attach(view.get());
