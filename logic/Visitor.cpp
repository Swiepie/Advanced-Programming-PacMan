...
const std::vector<Entity*>& CollectibleVisitor::getToRemove() const { return toRemove; }

GhostCollisionVisitor::GhostCollisionVisitor(std::shared_ptr<Pacman> pacman,  World* w, float tileWidth,
                                             float tileHeight)
    : pacman(std::move(pacman)), world(std::move(w)), stepW(tileWidth), stepH(tileHeight), pacmanDied(false), ghostEaten(false) {}
...
void GhostCollisionVisitor::visit(RedGhost& ghost) { handleGhostCollision(ghost); }
void GhostCollisionVisitor::visit(BlueGhost& ghost) { handleGhostCollision(ghost); }
void GhostCollisionVisitor::visit(PinkGhost& ghost) { handleGhostCollision(ghost); }

void GhostCollisionVisitor::handleGhostCollision(Ghost& ghost) {
    if (!pacman || ghostEaten)
        return;
...
void DeathVisitor::visit(RedGhost& ghost) { resetGhost(ghost); }
void DeathVisitor::visit(BlueGhost& ghost) { resetGhost(ghost); }
void DeathVisitor::visit(PinkGhost& ghost) { resetGhost(ghost); }
void DeathVisitor::resetGhost(Ghost& ghost) { ghost.reset(); }