...
CollectibleVisitor::CollectibleVisitor(std::shared_ptr<Pacman> pacman, World *w,
...
GhostCollisionVisitor::GhostCollisionVisitor(std::shared_ptr<Pacman> pacman,
                                             World *w, float tileWidth,
                                             float tileHeight)
    : pacman(std::move(pacman)), world(w), stepW(tileWidth), stepH(tileHeight),
...
void GhostCollisionVisitor::handleGhostCollision(Ghost &ghost) {
  ...
  if (pacman->collidesWith(ghost, stepW * 1.0f, stepH * 1.0f)) {
    if (world->getFearMode() && ghost.getFearState() &&
        !ghost.getHasBeenEaten()) {
      ghost.resetToSpawn();
...
CollisionDetectionVisitor::CollisionDetectionVisitor(float x, float y,
                                                     float tileWidth,
                                                     float tileHeight)
    : checkX(x), checkY(y), stepW(tileWidth), stepH(tileHeight),
...
bool CollisionDetectionVisitor::hasCollision() const {
