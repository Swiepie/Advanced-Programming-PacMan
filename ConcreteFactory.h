...
public:
  ConcreteFactory(std::shared_ptr<sf::RenderWindow> window, int height,
                  int width);
  ...
  std::unique_ptr<BlueGhost> createBlueGhost(float x, float y,
                                             float delay) override;
  std::unique_ptr<PinkGhost> createPinkGhost(float x, float y,
                                             float delay) override;
};
